...
router.post('/register', (req, res) => {
  ...
  User.findOne({ email: req.body.email }).then(user => {
    if (user) {
      ...
      //   d: 'mm' // Default
      // });
      fetch(`http://picasaweb.google.com/data/entry/api/user/${req.body.email}?alt=json`)
      .then(res =>{
      ...
      .then(user =>{
        let avatar=null;
        if(!user){
           avatar="http://localhost:5000/img/default.png";
        }
        else{
      avatar=user.entry.gphoto$thumbnail.$t; }
